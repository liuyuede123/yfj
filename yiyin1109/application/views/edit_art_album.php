...
<script src="plugins/webuploader/webuploader.artimg.config.js"></script>

<script>
	
function save_text(id){
	var text = $('#text_'+id).val();
	$.post(admin.url+'category_action/save_art_text',
	{id:id, text:text},
	function (){
		alert('保存成功');
		location.reload();
...
function del_picture(id){
	if(confirm('确认删除该图片？')){
		$.post(admin.url+'category_action/del_art_album',
		{id:id},
		function (){
			alert("删除成功");
			location.reload();
