var SuggestServer = function() {
		...
		this._fill = function() {
			...
			if ("key_" + a in this._objectData && "" != this._objectData["key_" + a]) {
				if (null == this._elementContainer) {
					...
					this._elementContainer.style.cssText += "display:none; filter:alpha(opacity=95); opacity:0.95; position:absolute; width:" + this._objectConfig.width + "px; z-index:999;height:410px;overflow-y:auto;overflow-x:hidden;";
				...
				for (c = 0; e > c; c++) {
					...
					if (null == this._objectConfig.link || "" == this._objectConfig.link) h = ['<td style="padding:0px;"><span style="display:block; padding:1px;">', "</span></td>"];
						...
						h = ['<td style="padding:0px;"><a href="' + i + '" hidefocus="true" onmousedown="return this.parentNode.parentNode.' + f + "['hidepause'](this);\" onclick=\"this.parentNode.parentNode." + f + '[\'hideresume\'](this);" style="color:#999; display:block; outline:none; padding:1px; text-decoration:none; width:100%;" target="' + this._objectConfig.target + '">', "</a></td>"]
		...
		this._eventBlur = function() {
			...
			this._stringLastValue = "请输入电话号码";
			this._hide()
		};
		
		//checked
		this._eventCheck = function(){
			this._elementInput.value == this._stringOriginalValue && (this._elementInput.value = "");
			this._stringLastValue = "";
			this._checkcode();
		}
		
		this._checkcode = function(){
			var a = this._elementInput.value;
			
				if ("" != a){
					if("key_" + a in this._objectData) {
						var code_key = eval("this._objectData.key_" + a);
						code_key = code_key.split(";");
						if(!code_key[0]){
							_isCheckCode = false;
						}
						for(i in code_key){
							if(code_key[i] != ""){
								code_key[i] = code_key[i].split(",");
								var realname = code_key[i][3];
								var chname = code_key[i][4];
								var name = code_key[i][2];
								//console.log(code_key[i]);
								if(realname == a){
									_isCheckCode = true;
								}else{
									_isCheckCode = false;
						...
						//this._fill();
						//console.log(eval(this._objectData.code_key."key_" + a));//////////////////////////////
					} else {
			...
			//下面开始检查表单
			var data = {};
			var post_url = 'submit_phone/add_phone';
			...
			data.zb = GetRequest();
			//console.log(data.zb);
			//data.sourseurl = window.location.href;
			if(_isCheckCode == false){
				/* var sMobile = data.phone; 
				if(!(/^1[3|4|5|8][0-9]\d{4,8}$/.test(sMobile))){ 
					this._success("请输入正确的股票代码和手机号码!"); 
				}else{ */
					this._success("请输入正确的股票代码！",0);
					this._eventBlur();
				//}
				return false;
			}else{
				/* var sMobile = data.phone; 
				if(!(/^1[3|4|5|8][0-9]\d{4,8}$/.test(sMobile))){ 
					this._success("请输入有效的手机号"); 
					return false; 
				}  */
				this._success("您的股票已提交，正在为您诊断，立即进入 QQ发送数字“1” 等待答复，谢谢参与！",1);
					this._eventBlur();
			}
			//提交表单
			/* $.post(
				...
				function(res){
					res = JSON.parse(res);
					//console.log(res);////////////////////////////
					(new SuggestServer())._success(res);
				}
			); */
			
		} 
		//
		this._success = function(res,flag){
			if(typeof(res) === "string"){
				_isSubmit = "showphone";
				$('.bdUpInputOK .lineb').html(res);
				if(!flag){
					settime = 1;
				}else{
					settime = 0;
				...
				if(!res.is_insert){
					_isSubmit = res.status?"showphone":"";
					$('.bdUpInputOK .lineb').html(res.msg);
				}else{
					_isSubmit = "showphone";
					$('.bdUpInputOK .lineb').html("请保持手机畅通,我们将安排投资顾问给您回电！");
				}
			}
			...
			if(settime){
				$(".bdUpInputOKBtn").hide();
				$('.bdUpInputOK').show();
				setTimeout(this._setTimeres,3000);
			}else{
				$(".bdUpInputOKBtn").show();
				$('.bdUpInputOK').show();
			}
		...
		this.bind = function(a) {
			...
			if (this._elementInput) {
				...
				//获取股票类型
				typeaaa = this._objectConfig.type;
				...
				///////////////
				//console.log($('.bdUpBtn'));///////////////
				this._aevent(document.getElementById("bdUpBtnid"), "click", this._bind(this._eventCheck));
